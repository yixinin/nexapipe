        ...
        F: FnOnce(&mut Context, (IpRepr, TcpRepr)) -> Result<(), E>,
    {
        ...
        match self.state {
            ...
            | State::LastAck => {
                ...
                let offset = if self.remote_last_seq >= self.local_seq_no {
                    self.remote_last_seq - self.local_seq_no
                } else {
                    // The remote ACKed beyond what we have sent (can happen with
                    // retransmissions or stale segments); nothing new to send.
                    0
                };
