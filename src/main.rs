use clap::Parser;
use nexapipe::config::{self, IrohConfig, ProxyConfig, ServerConfig};
use nexapipe::proxy::run_proxy;
...
async fn main() {
    ...
    if let Some(route_configs) = proxy_config.routes {
    ...
    if let Err(e) = run_proxy(routes, proxy_config.default_backend, server_config, iroh_config).await {
